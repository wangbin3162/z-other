<template>
  <div class="ratingselect">
    <div class="rating-type border-1px">
      ...
      <span @click="select(1,$event)" class="block nagative" :class="{'active':selectType===1}">{{desc.negative}}
  ...
  export default {
    ...
    methods: {
      select (type, event) {
        ...
        this.$emit('ratingtype-select', type);// 通知父组件更新数据
      },
      toggleContent (event) {
        if (!event._constructed) { // 如果是浏览器模式则阻止
          return;
        }
        this.$emit('toggle-content', this.onlyContent);// 通知父组件更新数据
      }
    }
  };
</script>
